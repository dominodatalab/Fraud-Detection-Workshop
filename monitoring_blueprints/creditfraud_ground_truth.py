"""Filled ground-truth generator for CreditFraudModel.

Matures the hidden fraud labels the load generator logged (older than the maturation window)
and writes ground-truth batches keyed on the capture row identifier. Registering the batch with
DMM for model-quality metrics needs an S3 datasource (DOMINO_PLATFORM_NOTES.md §5c) — the
fraud-detection-workshop instance has none, so batches land on disk for a dataset-based external
monitor / manual registration. Drift monitoring does not depend on this.
"""
...
def to_ground_truth(row: dict):
    """Map a matured scoring-log row to its ground-truth decision, in the SAME space as the
    model's `decision` prediction output (FRAUD/LEGIT)."""
    return "FRAUD" if int(row["true_label"]) == 1 else "LEGIT"
